...
class FreeEnergies(object):
    ...
    def average_gradients(self):
        r"""computes average gradients per lambda
            Returns
            -------
            gradient_averages : array
                array containing lambda value, average gradient and standard error
        """
        gradient_averages = []
        for g in range(len(self._gradients_kn)):
            avg = numpy.mean(self._gradients_kn[g])
            std_err = numpy.std(self._gradients_kn[g])/numpy.sqrt(len(self._gradients_kn[g]))
            gradient_averages.append(numpy.array([self._lambda_array[g],avg,std_err]))
        gradient_averages = numpy.array(gradient_averages)
...
class NotebookHelper(object):
    ...
    style = style)
        #fg = widgets.Checkbox(False, description='Generate Network analysis notebook',layout=layout, style = style)
        ui = widgets.VBox([a, b, c, d, e, f], layout=Layout(display='flex',
                        flex_flow='column',
                        align_items='stretch',
                        border='solid',
                        width='100%'))
        def _func(a,b,c,d,e,f):
            print((a,b,c,d,e,f))

        self._out = widgets.interactive_output(_func, {'a': a, 'b': b, 'c': c, 'd': d, 'e': e, 'f': f})
        return ui
    ...
    def run_free_energy_analysis(self, perturbation, files_input_bound, files_input_free, TI= False, separator = '~', average_gradients=False):
        ...
        average_gradients : boolean
            Default: False
            If set to true, will plot all average gradients for the free energy analysis being run
        ...
        if average_gradients:
            self._plot_average_gradients(perturbation, bound.average_gradients(), sim_type='bound')
        ...
        if average_gradients:
            self._plot_average_gradients(perturbation, free.average_gradients(), sim_type='free')
    ...
    def _write_overlap_info(self,perturbation, M, sim_type=''):
        ...
        ax = sbn.heatmap(M, annot=True, fmt='.2f', linewidths=.5, annot_kws={"size": 12})
    ...
    def _plot_average_gradients(self, perturbation, gradients, sim_type=''):
        ...
        """
        print(gradients)
        fplot = os.path.join(self._outputdir,perturbation)+'_'+sim_type+'_average_gradient.png'
        # turns off interactive plotting
        plt.ioff()
